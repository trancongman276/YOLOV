...
class YOLOXHead(nn.Module):
    def __init__(
        ...
        self.trans = MSA_yolov(
            dim=self.width, out_dim=4 * self.width, num_heads=heads, attn_drop=drop
        )
        self.stems = nn.ModuleList()
        self.linear_pred = nn.Linear(
            int(4 * self.width), num_classes + 1
        )  # Mlp(in_features=512,hidden_features=self.num_classes+1)
    ...
    def forward(self, xin, labels=None, imgs=None, nms_thresh=0.5):
        ...
        outputs_decode = torch.cat(
            [x.flatten(start_dim=2) for x in outputs_decode], dim=2
        ).permute(0, 2, 1)
        decode_res = self.decode_outputs(outputs_decode, dtype=xin[0].type())
        # Extract body feats
        # return decode_res
        return decode_res, before_nms_features, before_nms_regf, outputs_decode
    ...
    # #     topK=self.Afternum,
    # # )  # postprocess(decode_res,num_classes=30)
    ...
    # # features_cls, features_reg, cls_scores, fg_scores = self.find_feature_score(
    # #     cls_feat_flatten, pred_idx, reg_feat_flatten, imgs, pred_result
    # # )
    ...
    #         fc_output, [outputs_decode.shape[0], -1, self.num_classes + 1]
    #     )[:, :, :-1]
    ...
    #     # result, result_ori = postprocess(
    #     #     copy.deepcopy(pred_result), self.num_classes, fc_output, nms_thre=nms_thresh
    #     # )

    #     # return result, result_ori  # result
    #     return pred_result, fc_output
    ...
    def get_losses(
        ...
        for batch_idx in range(outputs.shape[0]):
            ...
            if num_gt == 0:
                ...
                ref_target = outputs.new_zeros(
                    (idx[batch_idx].shape[0], self.num_classes + 1)
                )
                ref_target[:, -1] = 1
                ...
                for ele_idx, ele in enumerate(idx[batch_idx]):
                    ...
                    if max_iou.values[ele_idx] >= 0.6:
                        ...
                        ref_target[ele_idx, : self.num_classes] = (
                            cls_target_onehot[max_idx, :] * max_iou.values[ele_idx]
                        )
                        fg += 1
                    else:
        ...
        loss_ref = (
            self.bcewithlog_loss(
                refined_cls.view(-1, self.num_classes)[ref_masks],
                ref_targets[ref_masks],
            )
        ).sum() / num_fg
    ...
    def postpro_woclass(
        self, prediction, num_classes, nms_thre=0.75, topK=75, features=None
    ):
    ...
    def postprocess_single_img(
        self, prediction, num_classes, conf_thre=0.001, nms_thre=0.5
    ):
        ...
        for i, detections in enumerate(prediction):
            ...
            conf_mask = (
                detections_ori[:, 4] * detections_ori[:, 5] >= conf_thre
            ).squeeze()
